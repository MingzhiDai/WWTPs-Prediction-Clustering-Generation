...
st.title("🧠 DE-BP Neural Network Genetic Algorithm Optimization (Include Generated Data)")
...
filename = st.text_input("输入原文件名（含路径）Input original filename (include file path)", "D:/BaiduNetdiskDownload/Sample_information_final-Phylums.csv")
# file_name_generate =  st.text_input("输入再生成文件名（含路径）Input generated filename(include file path)", "D:/BaiduNetdiskDownload/Generated_data_Phylums.csv")
...
output_dir = st.text_input("输出文件夹（路径）Output filename (include file path)", "D:/BaiduNetdiskDownload/Data_2/Phylum")
test_instance = st.number_input("测试实例组序号 Test instance group number: M", min_value=1, value=1, step=1)

st.markdown("------")

# --- 运行按钮 ---
if st.button("▶️ Run Generated DE-BP model"):
    ...
    try:
        cmd = [
            "python", "gabp+generative.py",
            "--file", filename,
            # "--generated_file", file_name_generate,
...
if st.session_state["Run_finished"]:
    ...
    # --- Result_Generative.txt ---
    if os.path.exists(output_path):
        st.write("📄 Result_Generative.txt Preview：")
        with open(output_path, "r", encoding="utf-8") as f:
            st.text(f.read()[:1000])
        with open(output_path, "rb") as f:
            st.download_button("📥 Download Result_Generative.txt", f, file_name="Result_Generative.txt", key="dl_txt")
    else:
        st.warning("❌ Can‘t find Result_Generative.txt file.")

    # --- Gabp_log.xlsx ---
    if os.path.exists(log_path):
        st.write("📊 Gabp_log(Generative).xlsx Preview：")
        try:
            df_log = pd.read_excel(log_path)
            st.dataframe(df_log.head(10))
        except Exception as e:
            st.error(f"Can‘t find Excel file：{e}")
        with open(log_path, "rb") as f:
            st.download_button("📥 Download Gabp_log(Generative).xlsx", f, file_name="Gabp_log(Generative).xlsx", key="dl_xlsx")
    else:
        st.warning("❌ Can‘t find Gabp_log(Generative).xlsx file.")

    # --- Test_instance_output.csv ---
    if os.path.exists(csv_path):
        st.write("📊 Test_instance_output(Generative).csv Preview：")
        try:
        ...
        with open(csv_path, "rb") as f:
            st.download_button(
                "📥 Download Test_instance_output(Generative).csv",
                f,
                file_name="Test_instance_output(Generative).csv",
                key="dl_csv"
            )
    else:
        st.warning("❌ Can‘t find Test_instance_output(Generative).csv file.")
    ...
    st.info("👆 Click the button above to run gabp+generative.py and view the results.")
