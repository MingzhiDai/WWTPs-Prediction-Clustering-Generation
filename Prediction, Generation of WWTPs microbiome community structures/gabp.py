...
# train_set = data_set[int(M * 0.2):, :]
# test_set = data_set[:int(M * 0.2), :]
train_set = np.vstack((
    data_set[:int(M * 0.6), :],  # 前 60%
    data_set[int(M * 0.8):, :]    # 后 20%
))
test_set = data_set[int(M * 0.6):int(M * 0.8), :]
...
# num_epoch = 1500  # 1500
# learn_rate = 1e-2
BP_net = BP_network.ini_BP_net(n_feature, n_hidden, n_output).to(device)  # 将模型移动到 GPU
x_train = train_set_norm[:, :n_feature]
y_train = train_set_norm[:, n_feature:].reshape(train_set_norm.shape[0], n_output)

# 格式转换并移动到 GPU
tensor_tran = transforms.ToTensor()
x_train = tensor_tran(x_train).to(torch.float).reshape(x_train.shape[0], n_feature).to(device)
y_train = tensor_tran(y_train).to(torch.float).reshape(y_train.shape[0], n_output).to(device)

# 进行训练
BP_lossList = BP_network.train(BP_net, num_epoch, learn_rate, x_train, y_train)

# 测试集
x_test = tensor_tran(test_set_norm[:, :n_feature]).to(torch.float).reshape(test_set_norm[:, :n_feature].shape[0], n_feature).to(device)
y_test = test_set_norm[:, n_feature:].reshape(test_set_norm.shape[0], n_output)

# 预测结果
BP_prediction = BP_net(x_test).detach().cpu().numpy()
# 拼接输入和预测结果
...
hidden_bias = best_code[n_feature * n_hidden: n_feature * n_hidden + n_hidden]
output_weight = best_code[n_feature * n_hidden + n_hidden: n_feature * n_hidden + n_hidden + n_hidden * n_output]
output_bias = best_code[n_feature * n_hidden + n_hidden + n_hidden * n_output: n_feature * n_hidden + n_hidden + n_hidden * n_output + n_output]
...
gaBP_net = BP_network.GABP_net(n_feature, n_hidden, n_output, GA).to(device)  # 将模型移动到 GPU
gaBP_lossList = BP_network.train(gaBP_net, num_epoch, learn_rate, x_train, y_train)

# 测试集预测
gaBP_prediction = gaBP_net(x_test).detach().cpu().numpy()
# 拼接输入和预测结果
...
y_test_ = tensor_tran(y_test).to(torch.float).reshape(y_test.shape[0], n_output).to(device)

# 计算误差
BP_error = loss_fc(BP_net(x_test), y_test_).detach().cpu().numpy() / n_output
gaBP_error = loss_fc(gaBP_net(x_test), y_test_).detach().cpu().numpy() / n_output
...
for i in range(n_output):
    plt.figure()
    plt.plot(gaBP_prediction_original[:, i], label='DE-BP prediction', c='r')
    plt.plot(test_set[:,n_feature:][:, i], label='Real data', c='b')
    plt.grid(ls='--')
    plt.legend()
